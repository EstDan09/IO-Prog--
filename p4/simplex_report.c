                             ...
                             SimplexTrace *trace)
{
    ...
    {
        ROW(T, cols, 0)
        [j] = (prob->sense == SIMPLEX_MAXIMIZE) ? -prob->c[j] : prob->c[j];
    }
    ...
    /* === Detección de múltiples soluciones ===
       Si existe NO-básica con costo reducido ~0 (original o holgura),
       hacemos un pivote extra para mostrar tabla alterna y x_alt. */
    if (result->status == SIMPLEX_OK)
    {
        ...
        if (is_basic)
        {
            ...
            /* Preferir variable original con costo reducido ~0; si no hay, aceptar holgura. */
            int alt_col = -1;
            for (int j = 0; j < n + m; ++j)
            {
            ...
            if (alt_col != -1)
            {
                ...
                if (Tc && basis_c)
                {
                    ...
                    /* Seleccionar fila saliente habitual (mínima fracción) para alt_col */
                    int leaving2 = -1;
                    double best = INFINITY;
                    for (int i = 1; i <= m; ++i)
                    {
                    ...
                    if (leaving2 != -1)
                    {
                        ...
                        /* Guardar tabla alterna, x_alt y z_alt */
                        ...
                        result->status = SIMPLEX_MULTIPLE; /* <-- marcar explícitamente */
                    }
                }
                free(Tc);
                free(basis_c);
    ...
    if (xpt)
    {
        for (int k = 0; k < 3; ++k)
        {
            ...
            for (int jj = 0; jj < n; ++jj)
                xpt[jj] = 0.0;
            /* Solo fijamos x_j=t si j es original; si es holgura, x_j no existe y no se imprime */
            if (j < n)
                xpt[j] = t;
            for (int i = 1; i <= m; ++i)
            {
